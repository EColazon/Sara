...
var (
	timeCountEleDataGet 	= 5
	timeCountEleCheckReg 	= 60 * 1
	timeCountEleSaveW 		= 60 * 30
	timeCountEleCheckAlarm	= 5
)
...
func HandleEleCollectManage() {
	...
	for {
		...
		if countEleDataGet > timeCountEleDataGet {
			countEleDataGet = 0
			handleGetDataEle()
		} else if countEleCheckReg > timeCountEleCheckReg {
			countEleCheckReg = 0
		...
		} else if countEleSaveW >= timeCountEleSaveW {
			countEleSaveW = 0
			// 获取供电模式
			// modePower = Redis.HandleRedisJsonGet(Shared.WDSysPowerMode)
			modePower := Redis.HandleRedisJsonGet(Shared.WDSysPowerMode)
		...
		} else if countEleCheckAlarm > timeCountEleCheckAlarm {
			countEleCheckAlarm = 0
